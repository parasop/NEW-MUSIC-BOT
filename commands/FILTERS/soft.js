...
if (!args[0]) {
			...
			const msg = await message.channel.send(`<a:MelodyM_loading:839504372815626301>Turning on **soft**.`);
	...
	player.setEQ(Array(13).fill(0).map((n, i) => ({
        ...
        const msg = await message.channel.send(`<a:MelodyM_loading:839504372815626301>Turning off **soft mode**.`);
			const embed = new MessageEmbed()
